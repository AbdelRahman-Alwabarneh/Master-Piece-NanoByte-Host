  ...
  try {
    ...
    const service = await Service.find({ userId: req.user.id })
      .populate({
        path: "OrderdId",
        select:
          "Servicetype orderStatus subscriptionDuration nextPaymentDate renewalFee paymentStatus serverId nextPaymentDate",
        populate: [
          {
            path: "serverId",
            select: "planName cpu ram storage planDescription planTitle",
          },
        ],
      })
      .sort({ createdAt: -1 })
  ...
  try {
    ...
    const service = await Service.findOne({ userId, _id: id, OrderNumber }) // البحث عن الخدمة بناءً على userId و _id
      .populate({
        ...
        select:
          "Servicetype orderStatus subscriptionDuration nextPaymentDate renewalFee paymentStatus vpsId dedicatedServerId",
        populate: [
          {
