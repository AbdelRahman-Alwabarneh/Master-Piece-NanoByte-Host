  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await Swal.fire({
        title: "هل أنت متأكد؟",
        text: "هل تريد إضافة هذا التوتوريال؟",
        icon: "warning",
        iconColor: "#ffcc00",
        background: "#18296C",
        color: "#ffffff",
      ...
      if (result.isConfirmed) {
        ...
        if (response.status === 201) {
          ...
          Swal.fire({
            title: "تمت الإضافة!",
            text: "تم إضافة التوتوريال بنجاح.",
            icon: "success",
            iconColor: "#28a745",
            background: "#18296C",
            color: "#ffffff",
            ...
            navigate(`/TutorialDetails/${newTutorialId}`);
          });
        } else {
        ...
        <div className="max-w-full mx-auto">
          <div className="bg-blue-950 bg-opacity-30 hover:shadow-blue-800/10 rounded-lg p-4 shadow-lg transition-shadow duration-300 hover:shadow-xl">
            <div className="flex justify-between border-b border-blue-700 pb-2 mb-4">
              <h2 className="text-lg font-semibold">إضافة محتوى تعليمي جديد</h2>
            </div>
